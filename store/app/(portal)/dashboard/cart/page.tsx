      ...
      <div className="space-y-4">
        ...
        {resumeCart && !clientSecret && (
          <p className="rounded border p-3 text-sm">
            Already attempted payment? <Link className="underline" href={`/dashboard/cart/thank-you?cart_id=${encodeURIComponent(resumeCart)}`}>Check that order before paying again.</Link>
          </p>
        )}
        {!cart.length ? (
          <Card
            ...
            headerClassName="min-h-0 flex-col items-stretch gap-3 px-3 py-3 sm:h-12.75 sm:flex-row sm:items-center sm:px-6 sm:py-4"
            titleClassName="text-base font-normal leading-tight"
          >
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Link
                ...
                className="mt-4 inline-flex items-center justify-center rounded-lg bg-neutral-900 px-4 py-2 text-xs font-medium text-white transition hover:bg-neutral-800"
              >
                Browse products
              </Link>
            </div>
          ...
          <>
            <div className="space-y-3">
              {cart.map(item => (
                <div key={item.product.id} className={quote ? 'pointer-events-none opacity-75' : ''} inert={!!quote}>
                  <CartItemCard item={item} onUpdateQuantity={updateQuantity} onRemove={removeFromCart} />
                </div>
              ))}
            </div>
            {!quote ? (
              <form onSubmit={prepare} className="space-y-5 rounded-lg border bg-white p-6 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_1px_2px_-1px_rgba(0,0,0,0.08),0_2px_4px_rgba(0,0,0,0.04)]">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {addressFields.map(([key, label]) => (
                    <label key={key} className="block text-sm">
                      ...
                      <input
                        name={key}
                        required={!['address_2', 'province'].includes(key)}
                ...
                <label className="block text-sm">
                  ...
                  <input name="promo_code" maxLength={100} className="mt-1 block w-full rounded border p-3" />
                </label>
                <label className="block text-sm">
                  Delivery notes (optional)
                  <textarea name="delivery_notes" maxLength={1000} className="mt-1 block w-full rounded border p-3" />
                </label>
                <p>Product subtotal: <strong>{formatBdt(subtotal)}</strong></p>
                <button disabled={busy} className="rounded bg-neutral-900 px-5 py-3 text-white disabled:opacity-50">
                  {busy ? 'Checking availability…' : 'Review order'}
                </button>
              </form>
            ) : (
              <div className="space-y-5 rounded-lg border bg-white p-6 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_1px_2px_-1px_rgba(0,0,0,0.08),0_2px_4px_rgba(0,0,0,0.04)]">
                    ...
                    >
                      {quote.shipping_options?.map(option => (
                        <option key={option.id} value={option.id}>
                          {option.name}{option.amount != null ? ` — ${formatBdt(Number(option.amount))}` : ''}
                        </option>
                      ))}
                    </select>
                  </label>
                ...
                {totals && (
                  <dl className="space-y-2">
                    ...
                    {clientSecret && (
                      <>
                        <div className="flex justify-between"><dt>Delivery</dt><dd>{formatBdt(Number(totals.shipping_total))}</dd></div>
                ...
                {clientSecret && totals ? (
                  <StripePayment cartId={totals.id} clientSecret={clientSecret} />
                ) : (
                  <div className="flex gap-4">
                    <button disabled={busy} onClick={() => setQuote(null)} className="rounded border px-5 py-3">Edit order</button>
                    <button disabled={busy || !shippingId} onClick={startPayment} className="rounded bg-neutral-900 px-5 py-3 text-white disabled:opacity-50">
